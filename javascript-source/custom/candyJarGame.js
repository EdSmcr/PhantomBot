...
(function() {
        ...
        if (command.equalsIgnoreCase('candyjar')) {
            if (!action) {
                ...
                if (action.equalsIgnoreCase('fill')) {
                    ...
                    if(sender == 'karlicartoons')
                    {
                            $.say('Sorry ' + username + ' but your permissions to fill the jar has been revoked. Please contact your administrator :-)');
                            return;
                    }
    ...
    $.bind('initReady', function() {
        if ($.bot.isModuleEnabled('./custom/candyJarGame.js')) {
            $.registerChatCommand('./custom/candyJarGame.js', 'candyjar', 7);
