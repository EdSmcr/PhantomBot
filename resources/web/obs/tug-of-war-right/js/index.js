...
$(function() {
    ...
    const updateMeter = function(obj) {
        ...
        JSON.parse(obj.data).map(json => {
            percentage = json.rightpercentage;
            tier = json.righttier;
            animate = json.rightanimate;
        });
        
        if (animate){
            popGun.play();
            $('#gun').attr('src', 'tug-of-war-right/images/right-tier-'+ tier +'.png').addClass('slide-in-elliptic-top-fwd').delay(1000).queue(function(){
                $(this).removeClass("slide-in-elliptic-top-fwd").dequeue();
            });
        }
        else{
            $('#gun').attr('src', 'tug-of-war-right/images/right-tier-'+ tier +'.png');
        }
